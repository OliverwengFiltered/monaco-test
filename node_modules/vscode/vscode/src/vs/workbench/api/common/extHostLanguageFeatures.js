...
import { CodeActionKind, Disposable, SyntaxTokenType, DocumentSymbol, Range as Range$2, InternalDataTransferItem, SemanticTokens, SemanticTokensEdits, SemanticTokensEdit, SnippetString, Location, CompletionList, InlineCompletionTriggerKind } from './extHostTypes.js';
    ...
    async provideMappedEdits(resource, codeBlocks, context, token) {
        ...
        const usedContext = ( context.documents.map((docSubArray) => ( docSubArray.map((r) => {
            return {
                ...
                version: r.version,
                ranges: ( r.ranges.map((range) => Range.to(range))),
            };
        }))));
        const ctx = {
